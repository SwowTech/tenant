...
/**
 * Enable swowtech/xunlian-trace-tp6 for a tenant.
 * Usage: php scripts/ensure-xunlian-trace-tp6-app.php --tenant=1
 */
...
if ($tenantId <= 0) {
    fwrite(STDERR, "Usage: php scripts/ensure-xunlian-trace-tp6-app.php --tenant=ID\n");
    exit(1);
}

$appDir = BASE_PATH . '/apps/swowtech/xunlian-trace-tp6';
if (! is_file($appDir . '/app.json')) {
    fwrite(STDERR, "FAIL: app not found at apps/swowtech/xunlian-trace-tp6\n");
    exit(1);
}

/** @var AppInstallService $install */
$install = ApplicationContext::getContainer()->get(AppInstallService::class);
$install->enableForTenant($tenantId, 'swowtech/xunlian-trace-tp6', '1.0.0');
echo "OK enabled swowtech/xunlian-trace-tp6 for tenant {$tenantId}\n";
