...
/**
 * Enable swowtech/yiqicms for a tenant.
 * Usage: php scripts/ensure-yiqicms-app.php --tenant=1
 */
...
if ($tenantId <= 0) {
    fwrite(STDERR, "Usage: php scripts/ensure-yiqicms-app.php --tenant=ID\n");
    exit(1);
}

$appDir = BASE_PATH . '/apps/swowtech/yiqicms';
if (! is_file($appDir . '/app.json')) {
    fwrite(STDERR, "FAIL: app not found at apps/swowtech/yiqicms\n");
    exit(1);
}

/** @var AppInstallService $install */
$install = ApplicationContext::getContainer()->get(AppInstallService::class);
$install->enableForTenant($tenantId, 'swowtech/yiqicms', '1.0.0');
echo "OK enabled swowtech/yiqicms for tenant {$tenantId}\n";
