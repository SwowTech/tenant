...
class UserService
{
    ...
    //发货权限
    public static function getSendPrivilege($user){
        return 1;
    }
    //打包权限
    public static function getBoxPrivilege($user){
        return 1;
    }
    //    记录权限
    public static function getRecordPrivilege($user){
        return 1;
    }
    //    核销权限
    public static function getCheckOffPrivilege($user){
        return 1;
    }
    ...
    {
        $data = CategoryModel::with('imgs')->where('is_visible', 1)->order(['sort'=>'desc','id'])->select();
...
//        $data['rate'] = RateModel::where('order_id', $id)->find();
        if ($data['yz_code']) {
            $data['qrcode'] = (new QrcodeService())->getQrcode($data['yz_code']);
        }
        if ($tui_data) {
            $data['tui'] = $tui_data;
        }
