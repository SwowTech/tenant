...
//        return $this->fetch('', ['data' => $data]);
    ...
    public function createData()
    {
        ...
        if ($this->request->isPost()) {
            ...
            // 导入sql数据并创建表
            $vaethink_sql = file_get_contents(VAE_ROOT . '/data/xunlian.sql');
            if ($vaethink_sql === false || $vaethink_sql === '') {
                throw new BaseException(['msg' => '安装SQL不存在: data/xunlian.sql']);
            }
            ...
            $create_admin_sql = "INSERT INTO " . "`".$data['DB_PREFIX'] . "admin"."` "
                . "(`id`,`username`,`password`,`group_id`,`ip`,`state`,`description`,`login_time`,`create_time`,`mcid`) VALUES "
                . "(1,'$username','$password',1,'',0,'',0,'$create_time',1)";
...
//                throw new BaseException(['msg' => '创建管理员信息失败']);
//            }
            if (false == file_put_contents(VAE_ROOT . "data/install.lock", 'xunlian安装鉴定文件，勿删！！！！！此次安装时间：' . date('Y-m-d H:i:s', time()))) {
                throw new BaseException(['msg' => '创建安装鉴定文件失败，请检查目录权限']);
            }

            (new QrcodeService)->getCode($_SERVER['HTTP_HOST'].'/h5/index.html'); 
            return $res['code'] = 1;
        }
