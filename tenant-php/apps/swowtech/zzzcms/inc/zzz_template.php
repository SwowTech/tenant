...
class ParserTemplate {
	var $tag,$searchmethod,$searchkeys,$searchcol,$searchtype,$searchsort,$page;
	...
	function parserlocation( $zcontent ) {
		...
		switch ( $location ) {
			...
			case 'taglist':
				$this->tag= G( 'tag' );
				$tag = db_select( 'tag', 't_name', "t_enname='" . G( 'tag' ) . "'" );
	...
	function parserContent( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			if ( conf( 'runmode' ) != 1 && $value[ 'c_gid' ] > 0 ) {
				$uid=get_session( 'uid' );
				if(!$uid) error( '401,请登录会员,您未登录或登录超时', '?user/login' );
				$gmark = get_session( 'gmark' ) ? get_session( 'gmark' ) : 0;
				$group = db_load_one( 'user_group','gid=' . $value[ 'c_gid' ], 'gid,g_name,g_mark' );
				$gmark < $group[ 'g_mark' ]and error( '401,会员级别不符,请升级到' . $group[ 'g_name' ] . '方可访问！' );
			}
			$cid = $value[ 'cid' ];
			$page = G('page',1) ;
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				switch ( $matches[ 1 ][ $i ] ) {
					...
					case 'pagedesc':										
						...
						break;
					case 'desc':                       
						$zcontent = str_replace( $matches[ 0 ][ $i ], $desc and '', $zcontent );
						break;
					case 'info':
			...
			for ( $i = 0; $i < $count; $i++ ) {
				switch ( $matches[ 1 ][ $i ] ) {
					...
					case 'wapkf':
						...
						break;
					case 'baidumap':
						$baidumap=conf('baidumapAK') ?load_file( SITE_DIR . 'plugins/baidumap.html' ) : '<a href="https://map.baidu.com/search/?querytype=s&da_src=shareurl&wd={zzz:companyname}" target="_blank"  title="未设置地图Key">百度地图</a>';
						$zcontent = str_replace( $matches[ 0 ][ $i ], $baidumap, $zcontent );
						break;
					case 'gaodemap':
						$gaodemap=conf('baidumapAK') ?load_file( SITE_DIR . 'plugins/gaodemap.html' ) : '<a href="https://gaode.com/search?query={zzz:companyname}/" target="_blank" title="未设置地图Key">高德地图</a>';
						$zcontent = str_replace( $matches[ 0 ][ $i ], $gaodemap, $zcontent );
						break;
					case 'sitepath':
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				switch ( $matches[ 1 ][ $i ] ) {
					...
					case 'zid':
						...
						break;  
					case 'bid':
						$zcontent = str_replace( '{zzz:bid}', G( 'bid',0 ), $zcontent );
						break;  	 
					case 'sid':
						$zcontent = str_replace( '{zzz:sid}', G( 'sid',0 ), $zcontent );
						break;
					case 'sname':
					...
					case 'psid':
						...
						break;
					case 'pagetitle':
						$zcontent = str_replace( $matches[ 0 ][ $i ], G( 'pagetitle', $data[ 'sitetitle' ] ), $zcontent );
						break;
					case 'pagekey':
					case 'pagekeys':
	...
	function parserLoopLabel( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$params = parserParam( $matches[ 2 ][ $i ] );
				// echop($label); echop($params) ;    // 跳过未指定gid的列表
				$type = '';
				$title = '';
				$id = '';
				$brand = '';
				...
				switch ( $label ) {
					...
					case 'slide':
						...
						$where ="slideonoff = 1 and (slidestart<".time()." or slidestart is null or slidestart =0) and (slideend>".time()." or slideend is null or slideend =0)";
						if ( !empty( $type ) )   $where .= " and slideclass = '$type'";
						if ( !empty( $id ) )   $where .= " and slideid in ($id)";
					...
					case 'sort':
						...
						break;		
					case 'custom':
						$order = array( 'customorder' => $asc, 'customid' => $asc );
						$where = array( "customonoff" => 1 );
						if ( !empty( $id ) )arr_add( $where, 'customid', $id );						
						$custom = db_load_one( "content_custom", $where );
						$options=splits($custom['customoptions']);
						$data=[];
						foreach($options as $k=>$v){
							$data[$k]['title']=$custom['customname'];
							$data[$k]['name']=$v;
							$data[$k]['enname']=$custom['custom'];
						}					
						break;					
					case 'tag':
							...
							$params = parserParam( $matches2[ 2 ][ $j ] );                           
							 ...
							 switch ( $matches2[ 1 ][ $j ] ) {
                                    ...
                                    case 'date':
                                        ...
                                        break;
                                    case 'desc':
                                        $one_html = str_replace( $matches2[ 0 ][ $j ],  $value[ 's_desc' ] and '', $one_html );
                                        break;
                                    case 'num':
                                        $one_html = str_replace( $matches2[ 0 ][ $j ], db_count( 'sort', array( 'S_PID' => $value[ 'sid' ],'s_onoff'=>1 ) ), $one_html );
                                        break;
                                    case 'id':
                                    case 'sid':
                                        $one_html = str_replace( $matches2[ 0 ][ $j ], $value[ 'sid' ], $one_html );
	...
	function parserContentLoop( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				// 读取数据
				if ( is_numeric($nonum) )$colnum = intval($colnum) + intval($nonum);
	...
	function parserSearch( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$norecord =isset( $norecord ) ? str_replace('　',' ',$norecord) : '很抱歉，没有找到匹配内容！' ;
				$keywords=getform('keys','get');
				if($keywords){
					$this->searchmethod='get';
					$this->page =getform('page','get');
				}else{
					$this->searchmethod='post';
					$keywords=getform('keys','cookie');
					$this->page= G('page') ;					
				}
				$this->searchkeys=$keywords;
				...
				if ( $keys ) {					
					...
					$data = db_load( 'content c,sort s', $where, 'c.*,s_name,s_enname,s_filename,s_url', $size, $order, $this->page );				
							...
							for ( $j = 0; $j < $count2; $j++ ) { // 循环替换数据
								...
								switch ( $matches2[ 1 ][ $j ] ) {
									...
									case 'link':
										...
										break;									
									case 'down':
										$zcontent = str_replace( $matches2[ 0 ][ $j ], get_downurl( $value[ 'cid' ], $value[ 'c_downurl' ], $value[ 'c_downname' ] ), $zcontent );
										break;
									case 'downname':
										$zcontent = str_replace( $matches2[ 0 ][ $j ], arr_split( $value[ 'c_downname' ], ',', 0 ), $zcontent );
										break;	
									case 'downurl':
										$one_html = str_replace( $matches2[ 0 ][ $j ], arr_split( $value[ 'c_downurl' ], ',', 0 ), $one_html );
										break;	
									...
									case 'sname':
										...
										break;
									case 'sid':
										$one_html = str_replace( $matches2[ 0 ][ $j ], $value[ 'c_sid' ], $one_html );
										break;
									case 'slink':
										$one_html = str_replace( $matches2[ 0 ][ $j ], getsortlink( $value[ 'c_type' ], $value[ 'c_sid' ],  $value[ 's_filename' ], $value[ 's_url' ] ), $one_html );
										break;
	...
	function parserListPage( $zcontent ) {
		...
		switch ($location){
		  ...
		  break;
		  case 'search':
		  $url = getsearchlink($this->searchmethod,$this->searchkeys,$this->searchtype,$this->searchsort,$this->searchcol, '{page}');
		  break;
		  case 'taglist':
		  $url = gettaglink($this->tag, '{page}');
		  break;
		  default:	
			if(conf('runmode')==1){
				...
				switch($matches[ 1 ][ $i ] ){
					...
					case 'year':							
						...
						if(isset($params['type']) && isset($params['num'])){
							$years=$year.$params['type'].$params['num'];							
							$year= eval("return $years;");		
						}
						$zcontent =str_replace( $matches[ 0 ][ $i ],$year, $zcontent );
						break;	
					case 'location':	
