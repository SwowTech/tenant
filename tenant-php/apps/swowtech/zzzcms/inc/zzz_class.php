...
if (is_array($conf['db'] ?? null)) {
	...
	// 租户分表：cy_{id}_zzz_（由网关 X-Tenant-Prefix / X-Tenant-Id 注入）
	$tenantPrefix = '';
	if (! empty($_SERVER['HTTP_X_TENANT_PREFIX'])) {
		$tenantPrefix = (string) $_SERVER['HTTP_X_TENANT_PREFIX'];
	} elseif (getenv('X_TENANT_PREFIX') !== false && getenv('X_TENANT_PREFIX') !== '') {
		$tenantPrefix = (string) getenv('X_TENANT_PREFIX');
	} elseif (! empty($_SERVER['HTTP_X_TENANT_ID']) && (int) $_SERVER['HTTP_X_TENANT_ID'] > 0) {
		$tenantPrefix = 'cy_' . (int) $_SERVER['HTTP_X_TENANT_ID'] . '_';
	}
	if ($tenantPrefix !== '') {
		$conf['db']['tablepre'] = rtrim($tenantPrefix, '_') . '_zzz_';
	}
}
