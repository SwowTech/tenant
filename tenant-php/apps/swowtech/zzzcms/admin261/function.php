...
switch ($act) {
	...
	case 'selectsort':
		...
		break;
	case 'selectcustom':
		echo select_custom($type);
		break;
	case 'pinyin':
		echo pinyin($sort, '-');
		break;
	case 'checkname':
		echo db_count('content', "c_pagename='" . $sort . "'");
		break;
	case 'gettag':
		echo (tojson(db_load('tag', array('t_onoff' => 1), 't_name')));
		break;
}
function select_custom($type)
{
	$r = "<option value='c_addtime'>创建时间</option>
		<option value='c_title'>内容标题</option>
		<option value='c_title2'>短标题</option>
		<option value='c_content'>内容</option>
		<option value='c_onoff'>状态(0禁用,1启用)</option>
		<option value='istop'>置顶(0取消,1启用)</option>
		<option value='isgood'>推荐(0取消,1启用)</option>
		<option value='c_visits'>访问量</option>
		<option value='c_tag'>标签</option>
		<option value='c_gid'>会员组(0,4-8)</option>
		<option value='c_brand'>品牌</option>
		<option value='c_pagekey'>关键词</option>
		<option value='c_pagedesc'>描述</option>";
	$data = db_load('content_custom', ['customonoff' => 1, 'customtype' => ['LIKE' => $type]]);
	foreach ($data as $v) {
		$r .= "<option value='" . $v['custom'] . "'>" . $v['customname'] . "</option>";
	}
	return $r;
}
function aboutlist()
{
	$arr = array(); global $ai_conf;
	$where = '';
	$i = 0;
	$sid = safe_key(getform('sid', 'get'));
	...
	foreach ($data as $value) {
		$i = $value['aid'];
		if ($value['num'] > 0) {
			$ai_edit= $ai_conf['enable_ai']=="1" ?"<button type='button' class='btn btn-success dim' onClick=\"opennew('?act=ai&type=polish&table=about&id=".$i."')\"> <i class='fa fa-paint-brush'></i> AI 润色 </button>":"";
			array_push($arr, array(
				...
				'edit' => '<button type="button" class="btn btn-info dim" onclick="opennew(\'?act=about&aid=' .$i. '\')"><i class="fa fa-pencil"></i> 编辑</button> ' . $ai_edit,
			));
		}
	}
	return tojson($arr);
...
function brandlist()
{
	...
	foreach ($data as $value) {
		$i = $value['bid'];
		$ai_edit= $ai_conf['enable_ai']=="1" ?"<button type='button' class='btn btn-success dim' onClick=\"opennew('?act=ai&type=polish&table=brand&id=".$i."')\"> <i class='fa fa-paint-brush'></i> AI 润色 </button>":"";
		array_push($arr, array(
			'id' => $value['bid'],
			...
			'b_onoff' => "<button type='button'  onclick=setcol('b_onoff'," . abs($value['b_onoff'] - 1) . ",'" . $i . "')  id='" . $i . "b_onoff' class='btn btn-" . $value["b_onoff"] . "  dim'><i class='fa fa-toggle-on'></i></button>",
			'edit' => '<button type="button" class="btn btn-info dim" onclick="opennew(\'?act=brand&id=' . $i . '\')"><i class="fa fa-pencil"></i> 编辑</button> <button type="button" class="btn btn-danger dim" onclick="delid(' . $i . ',\'brand\')"><i class="fa fa-times"></i> 删除</button> ' . $ai_edit,
		));
	}
	return tojson($arr);
}
	...
	foreach ($data as $value) {
		...
		array_push($arr, array(
			...
			'<button type="button"  onclick=setcol("istop",' . abs($value['istop'] - 1) . ',"' . $i . '") id="' . $i . 'istop" class="btn btn-' . $value['istop'] . ' dim"><i class="fa fa-thumb-tack"></i></button>' .
				...
				'<button type="button"  onclick=setcol("isoffer",' . abs($value['isoffer'] - 1) . ',"' . $i . '") id="' . $i . 'isoffer" class="btn btn-' . $value['isoffer'] . ' dim"><i class="fa fa-tag"></i></button>',
...
function labelslist(){
	...
	foreach ($data as $value) {
		$i = $value['labelid'];		
		$ai_edit= $ai_conf['enable_ai']=="1" ?"<button type='button' class='btn btn-success dim' onClick=\"opennew('?act=ai&type=polish&table=labels&id=".$i."')\"> <i class='fa fa-paint-brush'></i> AI 润色 </button>":"";
		array_push($arr, array(
			'id' => $i,
			...
			'edit' => "<button type='button' class='btn btn-info dim' onclick=opennew('?act=labels&id=" . $i . "')><i class='fa fa-pencil'></i> 编辑</button> <button type='button'  onclick=delid('" . $i . "','labels')  class='btn btn-danger dim'><i class='fa fa-times'></i> 删除</button> ".$ai_edit,
...
function regionlist()
{
	...
	if ($search) {
		$where .= " and (title like '%" . $search . "%' or zipcode= '" . $search . "' or telcode= '" . $search . "' or zonecode= '" . $search . "')";
	}
	$data = db_load('region', $where, '*', $limit, $orderby, $page);
	$count = db_count('region', $where);
	foreach ($data as $value) {
		$i = $value['id'];
		array_push($arr, array(
			'id' => $i,
			...
			'order' => "<input type='number' class='order' id='r_order-" . $i . "' placeholder='" . $value["r_order"] . "'  value='" . $value["r_order"] . "'>",
			...
			'edit' => "<a href='?act=regionlist&pid=$value[rid]'>$sub</a> <a  onclick=opennew('?act=region&id=" . $i . "')>编辑</a> "
...
function loglist()
{
	...
	if (get_session('adminmark') > 10000) {
		$where = $uid ? "userid=$uid" : 'lid=1';
	} else {
		$where = "userid=" . get_session('adminid');
	}
	if ($sort) {
		$orderby = array($sort => $order, 'logid' => 'desc');
	} else {
		$orderby = ['logid' => 'desc'];
	}

	if ($search) {
		$where .= " and  or ldesc like '%" . $search . "%'";
	}
	$data = db_load('log_dbsql', $where, '*', $limit, $orderby, $page);
	$count = db_count('log_dbsql', $where);
	foreach ($data as $value) {
		$i = $value['logid'];
		$edit = get_session('adminmark' > 10000) ? "<button type='button'  onclick=delid('" . $i . "','log_dbsql')  class='btn btn-danger dim'><i class='fa fa-times'></i></button>" : '';
		array_push($arr, array(
			'id' => $i,
			'ip' => $value["ip"],
			'username' => $value["username"],
			'ldesc' => "<input type='text' class='title' id='ldesc-" . $i . "' placeholder='" . $value["ldesc"] . "'  value='" . $value["ldesc"] . "'>",
			'content' => "<input type='text' class='title' id='content-" . $i . "' placeholder='" . $value["content"] . "'  value='" . $value["content"] . "'>",
...
function userlist()
{
	...
	if ($sort) {
		$orderby = array($sort => $order, 'uid' => 'desc');
	} else {
		$orderby = ['balance' => 'desc', 'uid' => 'asc'];
	}

	if ($search) {
		$where .= " and (truename = '" . $search . "' or mobile= '" . $search . "')";
	}
	$data = db_load('user', $where, '*', $limit, $orderby, $page);
	$count = db_count('user', ['u_gid' => 4]);
	foreach ($data as $value) {
		$i = $value['uid'];
		array_push($arr, array(
			'id' => $i,
...
function adminlist()
{
	...
	foreach ($data as $value) {
		...
		if ($i == get_session('adminid')) {
			$onoff = "<button type='button'  class='btn dim'><i class='fa fa-toggle-on'></i></button>";
			$del = "";
			$log = "<a href='?act=loglist&uid=" . $i . "'  class='btn btn-default dim'><i class='fa fa-history'></i></a>";
		} else if (get_session('adminmark') > $value["g_mark"] || get_session('adminmark') > 9999) {
		...
		array_push($arr, array(
			...
			'u_order' => "<input type='number' class='order' id='u_order-" . $i . "' placeholder='" . $value["u_order"] . "'  value='" . $value["u_order"] . "'>",
			'u_onoff' => $onoff,
			'edit' => "<button type='button' class='btn btn-info dim' onclick=opennew('?act=admin&uid=" . $i . "')><i class='fa fa-pencil'></i></button> " . $del . $log
		));
	}
	return tojson($arr);
}
...
function slidelist()
{
	...
	foreach ($data as $value) {
		$i = $value['slideid'];
		if(!isset($value['slideend'])){
			db_add('slide',['slidestart'=>'int']);	
			db_add('slide',['slideend'=>'int']);
			$start=$value["slidestart"]=0;
			$end=$value["slideend"]=0;
		}else{
			$start=$value["slidestart"] ? date("Y-m-d H:i:s",$value["slidestart"]) : "0";
			$end=$value["slideend"] ? date("Y-m-d H:i:s",$value["slideend"]) : "0";
		}
		array_push($arr, array(
			'id' => $i,
			...
			'slidestart' => "<input type='text' class='title' id='slidestart-" . $i . "' placeholder='" .$value["slidestart"] . "'  value='" . $start . "'>",
			'slideend' => "<input type='text' class='title' id='slideend-" . $i . "' placeholder='" . $value["slideend"] . "'  value='" . $end . "'>",	
...
function customlist()
{
	...
	foreach ($data as $value) {
		...
		$editlink = $value['customtype'] == 'gbook' ? '?act=gbookcustom&customid=' . $i : '?act=custom&customid=' . $i;
		array_push($arr, array(
			'id' => $i,
...
function imagelist()
{
	...
	switch ($uporder) {
		case 'size1':
			array_multisort($size, SORT_DESC, SORT_STRING, $files);
			break;
		case 'size2':
			array_multisort($size, SORT_ASC, SORT_STRING, $files);
			break;
		case 'name1':
			array_multisort($name, SORT_DESC, SORT_STRING, $files);
			break;
		case 'name2':
			array_multisort($name, SORT_ASC, SORT_STRING, $files);
			break;
		case 'mtime2':
...
function filelist($str)
{
	...
	switch ($str) {
		case 'template':
			$type = $type == 'wap' ? 'wap' : 'pc';
			$folders = str_replace('/' . $str . '/' . $type . '/', '', $folders);
			$path = $type == 'wap' ? SITE_DIR . 'template/wap/' . $folders : SITE_DIR . 'template/pc/' . $folders;
			$files = path_list($path, "template");
			break;
		case 'upload':
		...
		case 'data':
			...
			break;
		case 'login':
			$default_path = SITE_PATH . 'runtime' . ADMIN_PATH . 'login/';
			if ($folders) {
				$path = $folders;
			} else {
				$path = $default_path;
		...
		case 'backup':
			...
			$file1 = getfiles($path, 'zip');
			$path = RUN_DIR . 'backup';
			$file2 = getfiles($path, 'zip');
			$file3=[];
			switch ($conf['db']['type']) {
				case 'sqlite':
					$path = SITE_DIR . $conf['db']['sqlitepath'] . 'backup';
					$file3 = getfiles($path, 'lite');
					break;
				case 'mysql':
					$path = ADMIN_DIR . 'backup';
					$file3 = getfiles($path, 'bak');
					break;
			}
	...
	if (!is_array($files)) {
		...
		foreach ($data as $value) {
			...
			$url = $value['url'];
			if (lenstr($url) < lenstr($default_path)) continue;
			$size = formatnum($value['size'], 'size');
			switch ($value['ext']) {
				case '':
					if ($value['title'] == '返回') {
						$edit = '<a href="?act=' . $str . 'list&type=' . $type . '&folder=' . $url . '" class="btn btn-default dim"><i class="fa fa-folder-open"></i></a>';
						$name = '<a href="?act=' . $str . 'list&type=' . $type . '&folder=' . $url . '"><i class="fa fa-folder"></i>&nbsp;' . $value['title'] . '(' . $value['count'] . ')</a>';
					} else {
						$edit = '<a href="?act=' . $str . 'list&type=' . $type . '&folder=' . $url . '" class="btn btn-default dim"><i class="fa fa-folder"></i></a>';
						$name = '<a href="?act=' . $str . 'list&type=' . $type . '&folder=' . $url . '"><i class="fa fa-folder"></i>&nbsp;' . $value['title'] . '(' . $value['count'] . ')</a>';
					}
					$ext = 'Folder';
					$size = '';
				...
				case 'gif':
				case 'bmp':
					$name = '<a href="' . $value['url'] . '" class="fancybox"  rel="group"><i class="fa fa-image"></i>&nbsp;' . $value['title'] . '</a>';
					$ext = 'Image';
					break;
				case 'txt':
				case 'xml':
				...
				case 'css':
				case 'zzz':
					$edit = '<a href="' . $value['url'] . '" class="btn btn-default dim" target="_blank"><i class="fa fa-external-link"></i></a> <a href="javascript:void(0)" class="btn btn-success dim" onclick=opennew(\'?act=codeedit&folder=' . $value['url'] . '\')><i class="fa fa-pencil"></i></a>' . $edit;
					$name = '<a href="javascript:void(0)" onclick=opennew(\'?act=codeedit&folder=' . $value['url'] . '\')><i class="fa fa-edit"></i>' . $value['title'] . '</a>';
					$ext = 'Text';
					break;
				case 'html':
				case 'tpl':
					$edit = '<a href="?act=templateedit&folder=' . $value['url'] . '" class="btn btn-success dim"><i class="fa fa-pencil"></i></a>' . $edit;
					$name =  '<a href="?act=codeedit&folder=' . $value['url'] . '"><i class="fa fa-edit"></i>&nbsp;' . $value['title'] . '</a>';
					$ext = 'Html';
					break;
				case 'acc':
				case 'lite':
				case 'bak':
					if (ifstrin($value['url'], 'backup')) {
						$edit = '<a href="javascript:void(0)" class="btn btn-warning dim" onclick=restore(\'' . $value['url'] . '\',\'' . $value['ext'] . '\')><i class="fa fa-recycle"></i></a>' . $edit;
					} else {
						$edit = '<a href="' . $value['url'] . '" class="btn btn-info dim")><i class="fa fa-down"></i></a>' . $edit;
					}
					$name =  $value['title'];
					$ext = 'Data';
					break;
				...
				default:
					$ext = $value['ext'];
					$name = '<a href="' . $value['url'] . '" download="' . $value['title'] . '"><i class="fa fa-' . $ext . '"></i> ' . $value['title'] . '</a> ';
					$edit = '<a href="' . $value['url'] . '" download="' . $value['title'] . '" class="btn btn-default dim"><i class="fa fa-external-link"></i></a> ' . $edit;
			}
			array_push($arr, array(
				'name' => $name,
