          ...
          <div class="dbType" id="mysql" style="display:none;">
            <table border="0" cellpadding="0" cellspacing="0" class="infoTable mssqlTable">
              ...
              <tr>
                ...
                <td class="infotd"><input type="text" class="text" id="name" name="name" value="<?php
                  $dbName = getenv('DB_DATABASE');
                  if (!is_string($dbName) || $dbName === '') { $dbName = (string)($conf['db']['name'] ?? 'mineadmin'); }
                  echo htmlspecialchars($dbName, ENT_QUOTES, 'UTF-8');
                ?>" placeholder="与宿主同库，勿单独建 zzzcms" />
                  <label class="normal nameinfo">使用宿主 .env 的 DB_DATABASE，无需单独创建 zzzcms 库</label></td>
                </tr>
              <tr>
