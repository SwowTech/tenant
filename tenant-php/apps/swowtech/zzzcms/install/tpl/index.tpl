  ...
  <div class="mainpanel">
    ...
    <div class="rightpanel">
      ...
      <div class="license">
        ...
        <p>4.支持mysql,sqlite数据库，建议使用宝塔环境，不建议使用phpstudy</p>
      </div>
        <h1>安装环境</h1>
      <table border="0" cellpadding="0" cellspacing="0" class="listTable">
        ...
        <tr class="data">
          ...
          <td>
            <?php
            if(check_extensions('pdo_mysql')==1){
              echo '<span class="green">支持</span>';
            }else{
              echo '<b class="red">不支持</b>';
            }
            ?>
            </td>        
          <td>支持</td>
        ...
        <tr class="data">
          ...
          <td>
            <?php
            if(check_extensions('pdo_sqlite')==1){
              echo '<span class="green">支持</span>';
            }else{
              echo '<b class="red">不支持</b>';
            }
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="listTable">
        <tr class="top">
         ...
         <tr class="data">
          <td>根目录</td>
          <td>{$num_ch is_write(SITE_DIR),'<span class="green">满足</span>','<b class="red">不支持</b>'}</td>
          <td>可写</td>
        </tr>
         <tr class="data">
          <td>安装目录</td>
          <td>{$num_ch is_write(SITE_DIR.'install'),'<span class="green">满足</span>','<b class="red">不支持</b>'}</td>
          <td>可写</td>
        </tr>
        <tr class="data">
          <td>配置文件/zzz_config.php</td>
          <td>{$num_ch is_write(SITE_DIR.'config/zzz_config.php'),'<span class="green">满足</span>','<b class="red">不支持</b>'}</td>
          <td>可写</td>
        </tr>
        <tr class="data">
          <td>缓存文件/runtime/</td>
          <td>{$num_ch is_write(SITE_DIR.'runtime'),'<span class="green">满足</span>','<b class="red">不支持</b>'}</td>
          <td>可写</td>
        </tr>
        <tr class="data">
          <td>上传文件夹/upLoad/</td>
          <td>{$num_ch is_write(SITE_DIR.'upload'),'<span class="green">满足</span>','<sbpan class="red">不支持</b>'}</td>
          <td>可写</td>
        </tr>
      </table>
    </div>
