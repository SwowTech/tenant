...
function banner(){
    $r=[];
    $data=db_load('slide',['slideonoff'=>1,'slideclass'=>'A'],'slideid,slidename,slidelink,slideimg',10,['slideorder'=>'asc','slideid'=>'asc']);
    foreach($data as $v){
        $r[]=['clickUrl'=>decode($v['slidelink']),'images'=>$v['slideimg']];        
    }
   echo tojson(["data"=>["ext"=>'',"itemList"=>$r]]);
}

function getregion(){
    $pid=safe_word(getform('pid','post'));
    $data= db_load( 'region', array('pid'=>$pid,'onoff'=>1), 'rid,pid,title,telcode,zipcode',50,['r_order'=>'asc','rid'=>'asc']);
    echo tojson($data); 
}
...
function gbooking() {
    ...
    if(!$conf['gbookmark']) returnmsg($type,0,'留言失败,留言功能未开启'); 
    ...
    $data = db_load( 'content_custom', "customType = 'gbook'", 'customname,custom,customplace' );
    foreach ( $data as $value ) {
        $val = safe_key( getform( $value[ 'custom' ], "post",$value[ 'customplace' ] ) );
...
function reg() {
    ...
    if(!conf('usermark')) returnmsg($type,0,'注册失败,会员功能未开启'); 
    $backurl = safe_url(getForm( 'backurl', 'post' ));
    ...
    $colarr = array( 'username' => $username, 'truename' => $truename,  'password' => md5_16( $password ), 'mobile' => $mobile, 'email' => $email, 'regtime' => $regtime, 'u_onoff' => conf( 'useronoff' ), 'u_gid' => 4, 'u_order' => 9, 'balance' => 0, 'points' => 0, 'logincount' => 0 );
...
function edituser() {         
    ...
    if(!conf('usermark')) returnmsg($type,0,'编辑失败,会员功能未开启'); 
    ...
    $backurl= safe_url(getForm( 'backurl', 'post','',SITE_PATH.'?user/'  ));
    ...
    $data = db_load( 'content_custom', "customtype = 'user'", 'customname,custom' );
    foreach ( $data as $value ) {
...
function login() {   
    ...
    if(!conf('usermark')) returnmsg($type,0,'登陆失败,会员功能未开启');      
    $backurl = safe_url(getForm( 'backurl', 'post' ));
    $check = array( 'lenmin' => 5, 'lenmax' => 16 );
    $username = safe_key( getForm( 'username', 'post', $check ) );
...
function loginin( $username ) {  
    $data = db_load_sql( "select uid,username,truename,tel,mobile,email,qq,face,gid,g_name,g_mark from [dbpre]user as a,[dbpre]user_group as b where a.u_gid=b.gid and isadmin=0 and (username ='" . $username . "' or mobile ='" . $username . "')" );
    foreach ( $data as $value ) {
        ...
        db_update( 'user', array('uid'=>$value[ 'uid' ]),array('lastlogintime'=>date( 'Y-m-d H:i:s' ),'lastloginip'=>ip(),'logincount+'=>1));
    }
}
...
function forget() {
    ...
    $backurl = empty( $backurl ) ? SITE_PATH .'?location=user&act=forget': str_replace( 'ＰＨＰ', 'php', $backurl );
    if ( $count > 0 ) {
        loginin( $mobile );
      ...
      returnmsg($type,1,'验证成功，请进入会员中心修改密码', $backurl);      
    } else {  
      returnmsg($type,0,'验证失败,请确认问题答案是否正确',  $backurl);      
    }
}
...
function editface() {
    ...
    if( $uid){
        ...
        returnmsg($type,0,'修改失败,登陆失效', $backurl );    
    }
}
?>