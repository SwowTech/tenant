...
class db_pdo_sqlite {
	...
	public function istable($table) {
		$table= db_tableby_to_sqladd($table);				
		$sql="select case when exists (select 1 from sqlite_master WHERE type='table' and name = '$table') then 1 else 0 end as num";
		$arr =$this->sql_find_one($sql);
		return !empty($arr) ? intval($arr['num']) : $arr;
	}
