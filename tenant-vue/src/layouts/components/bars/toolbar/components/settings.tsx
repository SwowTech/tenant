...
export default defineComponent({
  ...
  setup() {
    ...
    const keys = ref<Record<string, unknown>>({
      ...
      collapseSubMenuMode: settingStore.getSettings('subAside')?.collapseSubMenuMode || 'popup',
    })
          ...
          v-slots={{
            ...
            default: () => (
              <div class="pb-10">
                ...
                {divider(useTrans('mineAdmin.settings.subAsideSetting') as string)}
                <div class="mine-setting-description mine-setting-description--nowrap">
                  <div class="desc-label whitespace-nowrap shrink-0 !w-auto pr-2">
                    {useTrans('mineAdmin.settings.subAsides.collapseSubMenuMode')}
                  </div>
                  <div class="desc-value !w-auto flex-1 min-w-0 overflow-x-auto">
                    {tabsRender(
                      'collapseSubMenuMode',
                      [
                        { label: useTrans('mineAdmin.settings.subAsides.collapseModePopup') as string, value: 'popup' },
                        { label: useTrans('mineAdmin.settings.subAsides.collapseModeAside') as string, value: 'aside' },
                      ],
                      (v: string) => {
                        settingStore.getSettings('subAside').collapseSubMenuMode = v as 'popup' | 'aside'
                      },
                      'h-6 text-[12px] whitespace-nowrap flex-nowrap',
                    )}
                  </div>
                </div>
