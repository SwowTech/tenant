...
export default defineComponent({
  ...
  setup(props) {
    ...
    const index = menu?.path || String(menu?.name) || JSON.stringify(menu)
    ...
    function handleClick() {
      // 折叠态点一级主菜单：打开/切换并进入第一个子菜单
      if (
        rootMenu.props.collapse
        && hasChildren.value
        && level === 0
      ) {
        if (rootMenu.props.collapseSubMenuMode === 'aside') {
          rootMenu.openCollapseAside(index, menu)
        }
        else {
          // popup 模式：点击同样进入第一个子菜单
          rootMenu.navigateCollapseFirstChild(menu)
        }
        return
      }
      // 浮层弹出模式：父级靠 hover，点击不处理（非折叠）
      ...
      if (hasChildren.value) {
        ...
        // aside 常驻列表：点击叶子不关闭右侧面板
        if (!(rootMenu.props.collapse && rootMenu.props.collapseSubMenuMode === 'aside')) {
          rootMenu.closeCollapseAside()
        }
      }
    }
    ...
    const hideInlineSubMenu = computed(() => {
      // 折叠 + 右侧列表：一级父菜单的子项只在右侧面板展示，禁止在主菜单栏内联/Teleport 下拉
      return !!rootMenu.props.collapse
        && rootMenu.props.collapseSubMenuMode === 'aside'
        && level === 0
    })
    ...
    return () => (
      <>
        <Item
          ...
          expand={opened.value && !hideInlineSubMenu.value}
          onClick={() => handleClick()}
                      ...
                      menu.children && menu.children?.map((item: MineRoute.routeRecord) => (
                        <>
                          {(isVisibleMenuItem(item)) && (
                            <SubMenu
                              key={item.path || String(item.name) || JSON.stringify(item)}
                              unique-key={[...uniqueKey, item.path || String(item.name) || JSON.stringify(item)]}
                              menu={item}
