...
http.interceptors.request.use(
  ...
  async (config) => {
    ...
    // 创始人接口操作平台名册，不要带租户头，避免表前缀串台
    const isFounderApi = url.includes('/admin/founder')
    if (tenantId && !isFounderApi && config.headers && config.headers['X-Tenant-Id'] == null && config.headers['x-tenant-id'] == null) {
