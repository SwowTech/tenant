...
import systemCheckRoute from '@/router/static-routes/systemCheckRoute'
  ...
  () => {
    ...
    async function initRoutes(router: Router, routes: any[]) {
      ...
      Object.keys(plugins).map((name: string) => {
        ...
        // 隐藏 MineAdmin 官方「应用商店」主菜单（改用云应用商城）
        const pluginName = plugin.config?.info?.name || name
        if (pluginName === 'mine-admin/app-store' || name.includes('app-store')) {
          return
        }
    ...
    function getMineRootLayoutRoute(): RouteRecordRaw {
      ...
      return {
        ...
        children: [
          ...
          toRecordRawRoute(systemCheckRoute),
          toRecordRawRoute({
            path: '/:pathMatch(.*)*',
